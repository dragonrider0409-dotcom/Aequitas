...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if '/optimize' in p:
                ...
                def pr(p_):
                    return {'weights':dict(zip(avail,[round(float(w),6) for w in p_.weights])),
                            'ret':round(float(p_.exp_return),6),'vol':round(float(p_.volatilityatility),6),
            ...
            if '/backtest' in p:
                from engine_portfolio import rolling_backtest, ledoit_wolf_cov, nearest_pd, sample_cov
                import time
