    ...
    def do_POST(self):
        ...
        try:
            ...
            if '/optimize' in p:
                ...
                from engine_portfolio import kelly_fraction, kelly_multi_asset
                tickers  = [t.strip().upper() for t in b.get('tickers', ['SPY'])]
                period   = b.get('period', '2y')
                rf       = float(b.get('rf', 0.05))
                method   = b.get('method', 'continuous')
                max_lev  = float(b.get('max_leverage', 1.0))

                data   = yf.download(tickers, period=period, auto_adjust=True,
                                      progress=False, threads=True)
                closes = data['Close'] if hasattr(data.columns, 'levels') else data
                avail  = [t for t in tickers if t in closes.columns] if hasattr(closes, 'columns') else tickers
                if len(avail) == 0:
                    return send_err(self, 'No valid tickers', 422)

                closes = closes[avail].dropna() if len(avail) > 1 else closes.dropna()
                rets   = np.log(closes / closes.shift(1)).dropna()

                if len(avail) == 1:
                    col = avail[0] if avail[0] in rets.columns else rets.columns[0]
                    result = kelly_fraction(rets[col].values, rf=rf, method=method,
                                            max_leverage=max_lev)
                    result['ticker'] = avail[0]
                    result['multi']  = False
                else:
                    result = kelly_multi_asset(rets[avail], rf=rf, max_leverage=max_lev)
                    result['multi'] = True
