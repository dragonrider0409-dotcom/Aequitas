...
def kelly_fraction(
    ...
    """
    Kelly Criterion optimal position fraction for a single asset.

    Two formulations:
    ─────────────────
    DISCRETE  (win/loss game):
        f* = (p·b − q) / b
        where p = P(win), q = 1−p, b = avg_win / avg_loss
        Classic Kelly for binary outcomes.

    CONTINUOUS (log-normal returns, Thorp 1969):
        f* = (μ − rf) / σ²
        where μ = mean annual return, σ² = annual variance
        Exact for GBM; standard at quant funds.

    Both are implemented; default is continuous (more appropriate for equities).

    Half-Kelly (f*/2) is also returned — widely used in practice because
    full Kelly maximises long-run growth but has high short-term drawdowns.
    Full Kelly drawdown can exceed 50% during normal operation.

    Parameters
    ----------
    returns      : daily log-returns array
    rf           : annual risk-free rate (for continuous method)
    method       : "continuous" | "discrete"
    max_leverage : cap on f* (default 2.0 = 200% of capital)

    Returns
    -------
    dict with full_kelly, half_kelly, quarter_kelly, method, mu, sigma,
    win_rate, avg_win, avg_loss, win_loss_ratio, expected_growth_rate,
    and kelly_by_fraction table for the frontend slider.
    """
    ...
    wins  = r[r > 0]
    loss  = r[r < 0]
    p     = float(len(wins) / len(r)) if len(r) > 0 else 0.5
    q     = 1.0 - p
    b     = float(wins.mean() / abs(loss.mean())) if len(loss) > 0 and abs(loss.mean()) > 1e-10 else 1.0

    # ── Kelly fractions ──────────────────────────────────────────────────
    if method == "discrete":
        f_star = (p * b - q) / max(b, 1e-10)
    else:  # continuous (default)
        excess = mu_ann - rf
        f_star = excess / max(var_ann, 1e-10)

    # Cap at max_leverage and floor at 0 (no shorting via Kelly here)
    f_star = float(np.clip(f_star, 0.0, max_leverage))

    # ── Expected log-growth at different fractions ───────────────────────
    # g(f) = f·μ − 0.5·f²·σ²  (continuous approximation)
    fracs = np.arange(0.0, min(max_leverage, 3.0) + 0.05, 0.05)
    growth = [round(float(f * mu_ann - 0.5 * f**2 * var_ann) * 100, 4) for f in fracs]
    kelly_table = [{"fraction": round(float(f), 2), "growth_pct": g}
                   for f, g in zip(fracs, growth)]

    # Expected log-growth at full Kelly
    g_star = f_star * mu_ann - 0.5 * f_star**2 * var_ann
    ...
    return {
        ...
        "method":             method,
        # Return stats
        "mu_ann":             round(mu_ann * 100, 4),          # %
        "sigma_ann":          round(sig_ann * 100, 4),         # %
        "sharpe":             round((mu_ann - rf) / max(sig_ann, 1e-10), 4),
        # Discrete stats
        "win_rate":           round(p * 100, 2),               # %
        "avg_win_pct":        round(float(wins.mean()) * 100, 4) if len(wins) else 0,
        "avg_loss_pct":       round(float(abs(loss.mean())) * 100, 4) if len(loss) else 0,
        "win_loss_ratio":     round(b, 4),
        # Growth
        "expected_log_growth_pct": round(g_star * 100, 4),
        "growth_table":       kelly_table,
    }
...
def kelly_multi_asset(
    ...
    """
    Multi-asset Kelly allocation (Markowitz-Kelly equivalence).

    Optimal log-growth weights: w* = Σ⁻¹ · (μ − rf)
    Then normalise so |w|₁ ≤ max_leverage.

    This is equivalent to unconstrained mean-variance optimisation
    with risk aversion = 1 (log utility investor).
    """
    mu  = returns.mean().values * 252                     # annualised means
    cov = returns.cov().values * 252                      # annualised cov
    try:
    ...
    # Scale so maximum absolute weight = max_leverage
    abs_sum = np.sum(np.abs(w_raw))
    if abs_sum > max_leverage:
        w_raw = w_raw * (max_leverage / abs_sum)

    port_mu  = float(w_raw @ mu)
    port_var = float(w_raw @ cov @ w_raw)
    g_star   = port_mu - 0.5 * port_var

    per_asset = [kelly_fraction(returns.iloc[:, i].values, rf)
                 for i in range(returns.shape[1])]

    return {
        "weights":               np.round(w_raw, 6).tolist(),
        "tickers":               list(returns.columns),
        "portfolio_mu_ann_pct":  round(port_mu * 100, 4),
        "portfolio_vol_ann_pct": round(np.sqrt(port_var) * 100, 4),
        "expected_log_growth_pct": round(g_star * 100, 4),
        "per_asset_kelly":       per_asset,
    }
