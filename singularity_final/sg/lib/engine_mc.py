...
def mc_american_lsm(paths: np.ndarray, cfg: SimConfig) -> Dict[str, float]:
    """
    Longstaff-Schwartz (2001) American option pricer via least-squares regression.

    Algorithm:
      1. Simulate risk-neutral paths (shape: n_steps × n_sims, each column is a path).
      2. At expiry, set cash flows = intrinsic value.
      3. Work backwards: at each step, regress continuation value on basis functions
         of the current spot (1, S, S²) using only in-the-money paths.
      4. Exercise where intrinsic > fitted continuation value.
      5. Discount the first exercise cash flow per path back to t=0.

    Basis: {1, S, S²}  — Laguerre polynomials are common but quadratic monomial
    basis is numerically equivalent and simpler to implement.

    Returns price, standard error, and early-exercise premium vs European MC.
    """
    disc = _disc(cfg.r, cfg.T)
    n_steps, n_sims = paths.shape
    dt   = cfg.T / n_steps
    df   = np.exp(-cfg.r * dt)   # per-step discount factor

    K, opt = cfg.K, cfg.option_type
    ...
    # Cash-flow matrix — track the first (earliest) optimal exercise time
    cf = np.zeros((n_steps, n_sims))
    cf[-1] = intrinsic(paths[-1])

    for t in range(n_steps - 2, 0, -1):
        S_t  = paths[t]
        itm  = intrinsic(S_t) > 0          # only regress in-the-money paths

        if itm.sum() < 10:
            continue  # too few ITM paths — no regression possible

        # Discounted continuation values from the future
        # Scan forward from t+1 to find each path's first nonzero future CF
        # then discount back to t.
        future_cf = np.zeros(n_sims)
        for s in range(t + 1, n_steps):
            # Only update paths that haven't exercised yet
            not_yet = future_cf == 0
            future_cf = np.where(not_yet, cf[s] * np.exp(-cfg.r * (s - t) * dt), future_cf)

        # Basis: [1, S, S²] restricted to ITM paths
        S_itm  = S_t[itm]
        Y      = future_cf[itm]
        ...
        ex_val = intrinsic(S_itm)
        exercise = ex_val > cont_hat

        # Zero out any future CFs for paths we're exercising now
        exercise_full = np.zeros(n_sims, dtype=bool)
        itm_idx = np.where(itm)[0]
        exercise_full[itm_idx[exercise]] = True

        cf[t, exercise_full]       = intrinsic(S_t)[exercise_full]
        cf[t + 1:, exercise_full]  = 0.0   # cancel later CFs on exercised paths

    # Discount each path's CF to t=0
    discount_factors = np.array([np.exp(-cfg.r * s * dt) for s in range(n_steps)])
    path_values = (cf * discount_factors[:, None]).sum(axis=0)

    price = float(path_values.mean())
    se    = float(path_values.std(ddof=1) / np.sqrt(n_sims))
    bs_eur = black_scholes(cfg)
    ...
    return {
        ...
        "early_ex_premium":round(max(price - bs_eur, 0.0), 6),
        "method":          "Longstaff-Schwartz LSM (quadratic basis)",
    }
...
def run_full_suite(cfg: SimConfig | None = None) -> Dict[str, Any]:
    ...
    american_lsm = mc_american_lsm(gbm_rn, cfg)
    res["options"] = dict(bs=bs, eur_gbm=eur_gbm, eur_heston=eur_heston,
                           ...
                           american_lsm=american_lsm,
                           greeks_bs=greeks_bs, greeks_mc=greeks_mc)
    log.info(f"  American LSM = ${american_lsm['price']:.4f}  "
             f"early_ex_premium=${american_lsm['early_ex_premium']:.4f}")
