...
def engle_granger(price_y: np.ndarray, price_x: np.ndarray) -> Dict:
    ...
    return {
        ...
        "adf_tau":      adf["tau"],
        "adf_p":        adf["p_value"],
        "eg_pvalue":    adf["p_value"],   # alias for screener compatibility
...
# ══════════════════════════════════════════════════════════════════════════
#  S&P 500 SECTOR-AWARE UNIVERSE SCREENER
# ══════════════════════════════════════════════════════════════════════════

# S&P 500 liquid sector representatives — pre-seeded for instant scan
# without needing Wikipedia. Groups pairs within sectors first (more likely
# to be cointegrated) then cross-sector.
SP500_SECTORS = {
    "Technology":    ["AAPL","MSFT","NVDA","AMD","INTC","QCOM","TXN","AVGO","ADBE","CRM","ORCL","IBM"],
    "Financials":    ["JPM","BAC","GS","MS","WFC","C","BLK","SCHW","AXP","COF","USB","PNC"],
    "Healthcare":    ["JNJ","UNH","PFE","MRK","ABBV","TMO","ABT","LLY","AMGN","GILD","REGN","BMY"],
    "Energy":        ["XOM","CVX","COP","SLB","EOG","MPC","VLO","PSX","OXY","PXD","HAL","BKR"],
    "ConsumerDisc":  ["AMZN","TSLA","HD","MCD","NKE","SBUX","LOW","TJX","ROST","CMG","LULU","F"],
    "ConsumerStapl": ["WMT","PG","KO","PEP","COST","TGT","MO","PM","MDLZ","KHC","CL","GIS"],
    "Industrials":   ["HON","BA","CAT","GE","RTX","MMM","DE","LMT","NOC","GD","UPS","FDX"],
    "Communication": ["META","GOOGL","NFLX","DIS","CMCSA","T","VZ","TMUS","CHTR","ATVI","EA","TTWO"],
    "Utilities":     ["NEE","DUK","SO","AEP","EXC","SRE","D","PCG","ED","XEL","ES","WEC"],
    "RealEstate":    ["AMT","PLD","CCI","EQIX","PSA","SPG","O","WELL","EQR","AVB","DRE","VTR"],
    "Materials":     ["LIN","APD","ECL","SHW","FCX","NEM","NUE","VMC","MLM","DD","PPG","ALB"],
}


def scan_sp500_universe(
    tickers: list,
    period: str = "2y",
    min_half_life: float = 1.0,
    max_half_life: float = 60.0,
    sector_first: bool = True,
    max_pairs_to_test: int = 500,
) -> dict:
    """
    Screen a large ticker universe for cointegrated pairs.

    Strategy:
    1. If sector_first=True, prioritise same-sector pairs (higher hit rate).
    2. Cap total pairs tested at max_pairs_to_test for speed.
    3. Return ranked results sorted by half_life (shorter = more tradeable).

    This makes the pairs dashboard usable for live trading:
    rather than typing in guesses, you get a ranked list of
    statistically validated pairs ready for the backtest tab.
    """
    try:
        import yfinance as yf
        import pandas as pd
    except ImportError:
        return {"error": "yfinance not installed", "pairs": []}
    ...
    if len(avail) < 2:
        return {"error": "Fewer than 2 tickers available", "pairs": [], "tickers": avail}

    # Build ticker→sector map
    ticker_sector = {}
    for sector, members in SP500_SECTORS.items():
        for t in members:
            ticker_sector[t] = sector

    # Generate pair candidates — sector pairs first, then cross-sector
    all_tickers = list(closes.columns)
    n = len(all_tickers)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ti, tj = all_tickers[i], all_tickers[j]
            si = ticker_sector.get(ti, "Unknown")
            sj = ticker_sector.get(tj, "Unknown")
            if si == sj and si != "Unknown":
                same_sector_pairs.append((ti, tj, si))
            else:
                cross_sector_pairs.append((ti, tj, f"{si}/{sj}"))

    # Prioritise same-sector; fill remainder from cross-sector up to cap
    candidates = same_sector_pairs[:max_pairs_to_test]
    remaining  = max_pairs_to_test - len(candidates)
    if remaining > 0:
        candidates += cross_sector_pairs[:remaining]

    results = []
    for (ta, tb, sector_label) in candidates:
        pa = closes[ta].values.astype(float)
        pb = closes[tb].values.astype(float)

        try:
            eg = engle_granger(pa, pb)
            if eg.get('cointegrated', False):
                sp   = compute_spread(pa, pb, eg['beta'], eg['alpha'])
                ou   = fit_ou(sp)
                hl   = ou.get('half_life_days', ou.get('half_life', 999))
                if min_half_life <= hl <= max_half_life:
                    results.append({
                        ...
                        "ou_mu":       round(ou.get('mu', 0), 4),
                        "ou_sigma":    round(ou.get('sigma', 0), 4),
                        "ou_kappa":    round(ou.get('kappa', 0), 4),
                        "hurst":       round(hurst_exponent(sp), 4),
                        "same_sector": sector_label == ticker_sector.get(ta, ""),
                    })
        except Exception:
            continue

    # Sort by half_life ascending (shortest = fastest mean reversion = most tradeable)
    ...
    return {
        ...
        "n_found":         len(results),
        "tickers_loaded":  avail,
        "same_sector_tested": len(same_sector_pairs),
    }
